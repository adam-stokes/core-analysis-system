...
def encrypt(key, decrypted_str):
    ...
    decrypted_str += u' ' * ( 16 - len(decrypted_str) % 16 )
    encrypted_str = encryptor.encrypt(decrypted_str)

    # encode str
    encoded_str = urllib.quote(base64.b64encode(encrypted_str).replace("/",".").replace("+","_").replace("=","-"))
    return encoded_str
                                    
def decrypt(key, quoted_str):
    decryptor = AES.new(key[0:16])
    encoded_str = urllib.unquote(quoted_str)
